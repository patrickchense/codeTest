...
public class Main4 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            if (restrictions.containsKey(after)) {
                List<Integer> befores = restrictions.get(after);
                befores.add(before);
                restrictions.put(after, befores);
            } else {
        ...
        // calculation
        List<Integer> travels = new ArrayList<>(n);
        travels.add(0);
        int count = help(travels, restrictions, paths,  0);
        System.out.println(count);
    }

    private static int help(List<Integer> travels, Map<Integer, List<Integer>> restrictions, int[][] paths, int start) {
        if (travels.size() == paths.length) {
            return 0;
        }
        int min = Integer.MAX_VALUE;
        int next = -1;
        List<Integer> otherNexts = new ArrayList<>();
        for (int i = 0; i < paths.length; i++) {
            if (paths[start][i] < min && !travels.contains(i)) {
                if (!restrictions.containsKey(i)) {
                    min = paths[start][i];
                    next = i;
                } else {
        ...
        if (restrictions.containsKey(next)) {
            List<Integer> befores = restrictions.get(next);
            for (int index : befores) {
                if (!travels.contains(index)) {
                    // never travels
                    otherNexts.add(index);
                }
            }
        }
        if (otherNexts.isEmpty()) {
            travels.add(next);
            return min + help(travels, restrictions, paths, next);
        }
        else {
            int other_min = Integer.MAX_VALUE;
            int other_next = -1;
            for (int index : otherNexts) {
                int m = paths[start][index] + help(travels, restrictions, paths, next);
                if (other_min > m) {
                    other_min = m;
                    other_next = index;
                }
            ...
            if (next != -1) {
                travels.add(next);
                int m =  min + help(travels, restrictions, paths, next);
                if (m < other_min) {
                    return m;
                }
                else {
