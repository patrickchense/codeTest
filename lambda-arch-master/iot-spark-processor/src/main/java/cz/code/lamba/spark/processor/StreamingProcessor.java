package cz.code.lamba.spark.processor;
...
import cz.code.lamba.spark.util.IoTDataDeserializer;
import cz.code.lamba.spark.util.PropertyFileReader;
import cz.code.lamba.spark.dto.IoTData;
import cz.code.lamba.spark.dto.POIData;
...
public class StreamingProcessor implements Serializable {
    ...
    private void start() throws Exception {
        ...
        jssc.checkpoint(prop.getProperty("cz.code.lamba.spark.checkpoint.dir"));
...
//        Map<TopicPartition, Long> lastOffSet = Collections.EMPTY_MAP;
    ...
    private Map<TopicPartition, Long> getLatestOffSet(SparkSession sparkSession, Properties prop) {
        ...
        try {
            Dataset<Row> parquet = sparkSession.read()
                    .parquet(prop.getProperty("cz.code.lamba.hdfs") + "iot-data-parque");
    ...
    private Map<String, Object> getKafkaParams(Properties prop) {
        ...
        kafkaProperties.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, prop.getProperty("cz.code.lamba.kafka.brokerlist"));
        kafkaProperties.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        kafkaProperties.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, IoTDataDeserializer.class);
        kafkaProperties.put(ConsumerConfig.GROUP_ID_CONFIG, prop.getProperty("cz.code.lamba.kafka.topic"));
        kafkaProperties.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, prop.getProperty("cz.code.lamba.kafka.resetType"));
    ...
    private JavaInputDStream<ConsumerRecord<String, IoTData>> getStream(
        ...
        List<String> topicSet = Arrays.asList(new String[]{prop.getProperty("cz.code.lamba.kafka.topic")});
    ...
    private SparkConf getSparkConf(Properties prop, String[] jars) {
        return new SparkConf()
                .setAppName(prop.getProperty("cz.code.lamba.spark.app.name"))
                .setMaster(prop.getProperty("cz.code.lamba.spark.master"))
                .set("spark.cassandra.connection.host", prop.getProperty("cz.code.lamba.cassandra.host"))
                .set("spark.cassandra.connection.port", prop.getProperty("cz.code.lamba.cassandra.port"))
                .set("spark.cassandra.auth.username", prop.getProperty("cz.code.lamba.cassandra.username"))
                .set("spark.cassandra.auth.password", prop.getProperty("cz.code.lamba.cassandra.password"))
                .set("spark.cassandra.connection.keep_alive_ms", prop.getProperty("cz.code.lamba.cassandra.keep_alive"))
                ;
//                .setJars(jars);
    }
